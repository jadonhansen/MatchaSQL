window.addEventListener('DOMContentLoaded', (event)  => {
	let heading = `<strong class=\"notif-heading mb-2\">Loading chats...</strong>`;
...
function removeChat(chatterEmail) {
	...
	$.ajax({
		...
		error: function() {
			console.log('error deleting contact/chat: ', chatterEmail);
		}
	});
}
			...
			if (ret.contactArr.length !== 0) {
				ret.contactArr.forEach(element => {
					if (element.main_image) {
						if (element.bio) {
							chatArr += `<form action="matched_profile" method="post" id=${element.userID}>
											...
											<div class="alert alert-primary alert-dismissible fade show ml-4 mr-4" role="alert">
												...
												<button class=\"close\" type="button" onclick="removeChat('${element.username}')">
